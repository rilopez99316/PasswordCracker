...
def list_of_hash_lines(salted_passwords):
    content = salted_passwords.read()
    co_list = content.split("\n")
    return co_list
...
def hash(word, salt):
    word = str(word)
    return  (hashlib.md5(salt.encode() + word.encode()).hexdigest())
...
#sorts list by length of string
def sorting(list):
...
def single_word(hash_line_list, word_list):
    ...
    for i in range(len(hash_line_list)):
        for j in range(len(word_list)):
            #gets value
            word = word_list[j]
            #print(hash_line_list[i])
            hashed_password = get_hash(hash_line_list[i])
            salt = get_salt(hash_line_list[i])

            #if password found, its prints it with the username, password, and hash
            if (hash(word,salt) == hashed_password):
                print(get_user(hash_line_list[i]), word,hashed_password)
                ...
                list = [get_user(hash_line_list[i]), word,hashed_password]
...
def digits (hash_line_list):
    ...
    for i in range (len(hash_line_list)): #traverses shadowfile
        while (number < 99999999): #checks password until number is 8 digits long
            ...
            #if password found, its prints it with the username, password, and hash
            if (hash(number,salt) == hashed_password):
                print(get_user(hash_line_list[i]), number, get_hash(hash_line_list[i]))

                #adds hashline to a list of found hashed password to later update the list of hashed_list and improve complexity
                new_hash_list.append(hash_line_list[i])

                #adds it to a list of found passwords
                list = [get_user(hash_line_list[i]), number,get_hash(hash_line_list[i])]
                users_passwords.append(list)
        ...
        for j in range(len(word_list)):
            while(len(password) < 11):
                ...
                if (hash(password, salt) == hashed_password):
                    print(get_user(hash_line_list[i]), password, get_hash(hash_line_list[i]))

                    #adds hashline to a list of found hashed password to later update the list of hashed_list and improve complexity
                    new_hash_list.append(hash_line_list[i])

                    #adds it to a list of found passwords
                    list = [get_user(hash_line_list[i]), password, get_hash(hash_line_list[i])]
                    users_passwords.append(list)
...
def main():
    with open ('SaltedPassTable.txt') as salted_passwords, open('words.txt') as words:
        word_list = valid_words(words)
        hash_line_list = list_of_hash_lines(salted_passwords)
